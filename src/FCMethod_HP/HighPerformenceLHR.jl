...
@everywhere function poisson_pdf(mu, n0)
    sum_ = sum(log.(1:n0))
    return exp(n0 * log(mu) - mu - sum_)
end

@everywhere function getMuBest(n0::Int, bkg::Float64)
    #=
...
@everywhere function getSortedRatio(mu, bkg, n0_upper, check::Bool = false)
    ...
    if check == false
        ...
        for n0 in 0:n0_upper
            ...
            ratio_list[n0+1] = poisson_pdf(mu+bkg, n0)/poisson_pdf(mu_best+bkg, n0)
        ...
        ratio_list = ones(Int, n0_upper+1)
...
@everywhere function selectN0(mu, bkg, index_sorted_list, cfdent_level)
    # A sub-purcess of likelihoodRatio.
    ...
    while sum_of_hypo < cfdent_level
        sum_of_hypo += poisson_pdf(mu+bkg, index_sorted_list[i+1]-1)
...
@everywhere function likelihoodRatio(mu::Float64, bkg::Float64, cfdent_level::Float64, n0_upper::Integer = 100, probability::Bool = false)
...
function selectMuRegion(mu_list, n0_limit_list)
    ...
    for i in 1:(length_of_muList-1)
        if n0_limit_list[i+1, 1] != n0_limit_list[i, 1]
            # println(n0_limit_list[i, 1], "\t",mu_list[i])
...
@everywhere function constructBelt(bkg::Float64, mu_list, cfdent_level::Float64=0.9, n0_upper::Int=100)
    #=
    ...
    n0_limit_list = SharedArray{Int}(length_of_list, 2)
    @sync @distributed for i in 1:length_of_list
